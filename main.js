...
let path1 = "http://localhost/IWP_Project_2/IWP_Project_2/main_accounts.php";
function redirect(){
    ...
    sendHttpRequest("GET", path1).then((responseData)=> {
        ...
        if (responseData === "denied"){
            ...
            document.getElementById("pageTitle").innerHTML = "WhatsDown - " + responseData["username"];
            document.getElementById("currentUsername").innerHTML = "Hello '" + responseData["username"] + "'!";
            ...
            chatContainer.style.backgroundImage = "url("+responseData["back_image"].trim()+")";
...
originalHistoryListElem = document.getElementById("originalHistoryListElem"),
...
path2 = "http://localhost/IWP_Project_2/IWP_Project_2/add_search_users.php",
...
function updateHistoryList_client(){
    ...
    if (currUser === "" || currUser.length === 0){//If page still didn't fully load, return

        return;
    }

    //Send a request to the server to search for our given input in the chats table of the current user
    sendHttpRequest("POST", path2 ,[receiver_username, currUser, 2]).then((response)=>{
        
        response = JSON.parse(response);
            
        if (response === "empty"){//We don't have any recent activity available.
        ...
        else{//We have recent activity and we must display it

            if (response.toString() === historyTable_FromServer.toString()){//If the current results match the new ones, don't remove and add again
            ...
            for (let i = 0; i < response.length; i++){
                ...
                clone.innerHTML += "<div onclick = 'displayChat(this)' id = '" +temp["chat_name"]+ "'><img src='" + temp["image"] + "'> <label>" +  temp["username"]+ "</label></div>";
            }
        }
    });
}
...
historyBar.addEventListener("keyup", ()=>{
    ...
    if (historyBar.value === "" || historyBar.value.length === 0){
        ...
        setTimeout(() => {updateHistoryInterval = setInterval(updateHistoryList_client ,900)});
        return;
    }
    setTimeout(()=>{}, 100);
...
searchBar.addEventListener("keyup", ()=>{
    ...
    if (searchBar.value.length === 0 || searchBar.value === ""){
        ...
        sendHttpRequest("POST", path2, [searchBar.value, currUser, 0]).then( (response)=>{
...
function saveChat(userListElem){
    ...
    sendHttpRequest("POST",path2,[receiverName, currUser, 1, 1]).then((response)=>{
...
messages_receiverUser = document.getElementsByClassName("message_receiverUser"),
messages_currUser = document.getElementsByClassName("message_currUser"),
spoilerTag = document.getElementById("spoilerTag"),
path3 = "http://localhost/IWP_Project_2/IWP_Project_2/messages_retriever.php",
path4 = "http://localhost/IWP_Project_2/IWP_Project_2/sendMessages.php",
path5 = "http://localhost/IWP_Project_2/IWP_Project_2/saveFiles.php",
...
function updateChat(){
    ...
    if (currentChatTableName !== ""){//We don't accidently run this function to call the server to retrieve new messages

        //We send the request to the server
        sendHttpRequest("POST", path3, [currUser,currentChatTableName,lastDate]).then( (response) =>{

            response = JSON.parse(response);
         
            if (response !== "empty"){

                if (currentChatTableData !== response){//We don't change anything if the data is still the same


                    currentChatTableData = response;
                    
                    if (response["receiverName"] !== receiverName_chatSide.innerHTML){//A new receiver, we must change the displayed name
                        receiverName_chatSide.innerHTML = response["receiverName"];
                    }        

                    if (response["receiverImage"] !== receiverImage_chatSide.src){//a new receiver image, we must change it
                    ...
                    if (response["lastDate"] !== lastDate){//We might have new messages, we must display them

                        if (response["lastDate"] === "" || response["lastDate"].length === 0){//The server couldn't find any new messages.
                        ...
                        for (let i = 0; i < messages.length; i++){
                            ...
                            if (message["username"].trim() === currUser){//The current message was sent by the current user.

                                msgClass = "message_currUser";
                            }
                            else{//The current message was sent by the other user

                                msgClass = "message_receiverUser";
                            }
                            if (message["type"].trim() === "text"){//message is text

                                if (message["spoiler"].trim() === "0"){//Message is not a spoiler / NSFW

                                    chatContainer.innerHTML = chatContainer.innerHTML + "<div  class = '"+msgClass+"' id ='"+ JSON.stringify(message)+"'><label>" + message["message"] + "</label></div>";
                                    
                                }
                                else{//Message is a spoiler/ NSFW
                                    chatContainer.innerHTML = chatContainer.innerHTML + "<div class = '"+msgClass+" spoilerTextMessage hiddenTextMessage'  id='" + JSON.stringify(message) + "'>HIDDEN</div>";
                                }
                            }
                            else{//Message is image/file

                                if (message["spoiler"] === "0"){//Image is not a spoiler /NSFW

                                    chatContainer.innerHTML += "<div class = '"+ msgClass + " imgMessage' id = '"+ JSON.stringify(message)+ "'><img src = '" +message["message"]+"'></div>";
                                }
                                else{//Image is a spoiler /NSFW
                          
                                    chatContainer.innerHTML += "<div class = '"+ msgClass + " imgMessage spoilerImage hiddenImage' id = '"+ JSON.stringify(message)+ "'><img src = 'images/hidden.png'></div>";
                                }
                            }
                        }
                        addELS();
...
function displayChat(chatToBeDisplayed){
    ...
    removeChatMessages();
    updateChat();
    setTimeout(()=>{updateChatMessagesInterval = setInterval(updateChat, 900)} ,100);
...
function hideChat(chatCloseElem){
    ...
    sendHttpRequest("POST", path2, [receiverName, currUser, 1, 0]).then((response) =>{

        //ADD HERE CODE LATER FOR THE CHAT SIDE.
        if (receiverName_chatSide.innerHTML === receiverName){//Current Receiver was in the chat that was hidden
...
function sendMessage(messageType){
    ...
    if (spoilerTag.checked === true){//Message is tagged as spoiler

        spoiler = 1;
    }
    
    if (messageType === "text"){//Sending text messages

        if (messageBar.value === "" || messageBar.value.length === 0){//Message bar is empty, we're not sending anything
            return;
        }

        //This is specifically for sending text messages
        sendHttpRequest("POST", path4, [currentChatTableName, currUser, messageBar.value, "text", spoiler,receiverName_chatSide.innerHTML]).then((response)=>{
        ...
        inpTempBtn.addEventListener("change", () =>{
            ...
            if (files.length > 0){
                ...
                saveFiles(path5,formData).then((newFileName) => {
                    sendHttpRequest("POST", path4, [currentChatTableName, currUser, newFileName, "image", spoiler,receiverName_chatSide.innerHTML]).then((response2)=>{
                        updateChat();
                    });
                })
            }
...
function hide_showMessage(messageDiv){  
    ...
    if (messageDiv.classList.contains("spoilerTextMessage") || messageDiv.classList.contains("spoilerImage")){

        let messageData = JSON.parse(messageDiv.id);

        if (messageData["type"] ==="text"){//We have a text message to deal with.
    
            if (messageDiv.classList.contains("hiddenTextMessage")){//The message is hidden we need to display it
    
                messageDiv.innerHTML = messageData["message"];
                messageDiv.classList.remove("hiddenTextMessage");
            }else{//We must hide the message 
                messageDiv.classList.add("hiddenTextMessage");
                messageDiv.innerHTML = "HIDDEN";
            }
    
        }else{//Obviously, we have an image to deal with
           
            if (messageDiv.classList.contains("hiddenImage")){//We have to display the image (remove blur)
                
                messageDiv.innerHTML = "<img src = '" + JSON.parse(messageDiv.id)["message"] + "'>";
                messageDiv.classList.remove("hiddenImage");
            
            }else{//We have to hide the message
...
path7 = "http://localhost/IWP_Project_2/IWP_Project_2/settings_manager.php",
path8 = "http://localhost/IWP_Project_2/IWP_Project_2/savePfp.php",
path9 = "http://localhost/IWP_Project_2/IWP_Project_2/saveWallpaper.php";
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    checkbox.addEventListener('click', function() {
        ...
        sendHttpRequest("POST", path7, ["set",currUser, "theme", themeChange]).then( (response) => {
...
function setBackColor(color){
    ...
    sendHttpRequest("POST", path7, ["set",currUser, "back_color", color]).then( (response) => {
    ...
    inpTempBtn.addEventListener("change", () =>{
        ...
        if (files.length > 0){
            ...
            saveFiles(path8,formData).then((newFileName) => {
                sendHttpRequest("POST", path7, ["set",currUser, "image",newFileName]).then((response2)=>{
                    redirect();
                });
            })
        }
...
bioContainer.addEventListener("keyup", (k)=>{
    ...
    if (k.key === "Enter" || k.keyCode === 13){
        ...
        sendHttpRequest("POST", path7, ["set", currUser, "bio", bioContainer.value]).then((response) => {
    ...
    inpTempBtn.addEventListener("change", () =>{
        ...
        if (files.length > 0){
            ...
            saveFiles(path9,formData).then((newFileName) => {
                sendHttpRequest("POST", path7, ["set",currUser, "back_image",newFileName]).then((response2)=>{
                    redirect();
                });
            })
        }
...
resetDefault.addEventListener("click", (event)=>{
    ...
    sendHttpRequest("POST", path7, ["set",currUser, "image","images/pfpDefault.jpg"]).then((response1)=>{
        
        sendHttpRequest("POST", path7, ["set",currUser, "back_color","default"]).then((response2)=>{
        
            sendHttpRequest("POST", path7, ["set",currUser, "back_image","images/dam.jpg"]).then((response3)=>{
        
                sendHttpRequest("POST", path7, ["set",currUser, "theme","light"]).then((response4)=>{
        
                    sendHttpRequest("POST", path7, ["set",currUser, "bio","Hello There! I am using Whatsdown!"]).then((response4)=>{
